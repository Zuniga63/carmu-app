...
const CategoryChart = ({ annualReport, period, month }: Props) => {
  ...
  const getLabels = (
    ...
    if (period === ChartPeriod.annual)
      labels.push(...MONTHS.map(monthName => monthName.slice(0, 3)));
  ...
  const getCategories = (annualReport: IAnnualReport): Category[] => {
    ...
    const categoryReports = annualReport.categories.filter(
      report => report.category.level === 0
    );
    ...
    categoryReports.forEach(({ category, amount }) => {
      ...
      if (category.level === 0)
        categories.push({ id: category.id, name: category.name, pareto });
    });
      ...
      categories.forEach((category, index) => {
        ...
        monthlyReports.forEach(report => {
          const categoryReport = report.categories.find(
            report => report.category.id === category.id
          );
    ...
    if (monthlyReports.length > 0 && !isNaN(month) && month >= 0) {
      categories.forEach((category, index) => {
        ...
        while (date.isBefore(endDate)) {
          ...
          if (dailyReport) {
            const categoryReport = dailyReport.categories.find(
              report => report.category.id === category.id
            );
            if (categoryReport) {
  ...
  const buildMonthlyChartData = (categories: Category[]) => {
    ...
    const datasets = getMonthlyDatasets(
      categories,
      annualReport,
      Number(month)
    );
