  ...
  return (
    <div className="flex flex-col gap-y-4 rounded-lg border border-gray-300 bg-gray-200 p-4 shadow-lg dark:border-gray-dark dark:bg-header">
      ...
      {full && (
        <>
          {summary.cash ? (
            <InvoiceFormSummaryItem title="Efectivo" value={summary.cash} />
          ) : null}
          {summary.balance ? (
            <InvoiceFormSummaryItem title="Saldo" value={summary.balance} />
          ) : null}
          {summary.cashChange ? (
            <InvoiceFormSummaryItem title="Cambio" value={summary.cashChange} />
          ) : null}
        </>
      )}
    </div>
